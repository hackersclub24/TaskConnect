  ...
  return (
    <div className="mx-auto max-w-3xl py-8">
      <div className="mb-8 text-center">
        ...
        {avgRating && (
          <div className="mt-4 inline-flex items-center gap-2 rounded-lg bg-amber-500/10 px-4 py-2">
            <Star className="h-5 w-5 fill-amber-400 text-amber-400" />
            <span className="text-lg font-semibold text-amber-300">{avgRating}</span>
            <span className="text-sm text-slate-400">average ({reviews.length} reviews)</span>
          </div>
        )}
      </div>

      {/* Write your own review */}
      <div className="mb-10 rounded-2xl border border-slate-800/80 bg-slate-900/90 p-6 shadow-xl">
        <h2 className="mb-4 text-lg font-semibold text-slate-50">Write your review</h2>
        {!token ? (
      ...
      <h2 className="mb-4 text-lg font-semibold text-slate-50">Reviews from the community</h2>
      {loading ? (
        <p className="py-8 text-center text-slate-400">Loading reviews...</p>
      ) : reviews.length === 0 ? (
        ...
        <div className="space-y-4">
          {reviews.map((r) => (
            <div
              key={r.id}
              className="rounded-xl border border-slate-800/80 bg-slate-900/70 p-5 shadow-sm"
            >
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-1 text-sm font-medium text-slate-300">{r.rating}/5</span>
                </div>
              ...
              <p className="text-slate-300">{r.review_text}</p>
            </div>
          ))}
        </div>
      )}
